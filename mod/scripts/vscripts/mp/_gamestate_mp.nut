...
void function GameStateEnter_WinnerDetermined_Threaded()
{
	...
	if ( IsRoundBased() )
	{
		...
		*/
			...
			SetGameState ( eGameState.PickLoadout ) // Always do that, or player will not reset.
	}
	else
	{
...
{
	#if DEV
	printt( "CleanUpEntitiesForRoundEnd start!" )
	#endif
	...
	foreach ( entity player in GetPlayerArray() )
	{
		...
		#if DEV
		printt( "Player", player, " Alive?: ", IsAlive( player ).tostring() )
		#endif
		if ( IsAlive( player ) )
		{
			#if DEV
			printt( "Killing player", player )
			#endif
	...
	#if DEV
	printt( "CleanUpEntitiesForRoundEnd end!" )
	#endif
